...
@app.route('/IncomeByYear.vue', methods=['GET'])
def income_by_year():
    year_get_db = 'salaries.db'
    con = sqlite3.connect(year_get_db)
    cur = conn.cursor()

    cur.execute(
        'SELECT year, base_salary, overtime_pay, allowances, transport, expense_reimburse, income_other, FROM salaries.db'
    )
