...
import YearLineChart from './YearLineChart.vue'
    ...
    async function loadYearData () {
        try {
            ...
            incomeArr.value  = onlyMonthRows.map(r => r.income)
            deductArr.value  = onlyMonthRows.map(r => r.deduction)
            netArr.value     = onlyMonthRows.map(r => r.income - r.deduction)
        } catch (err) {
            console.error('年データ取得失敗', err)
        }

        labels.value = monthlyData.value.map(r => r.month)
        incomes.value = monthlyData.value.map(r => r.income)
        deductions.value = monthlyData.value.map(r => r.deduction)
        nets.value = monthlyData.value.map(r => r.income - r.deduction)
    }
