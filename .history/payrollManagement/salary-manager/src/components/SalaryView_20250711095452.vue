<template>
    <div v-if="detail" class="p-6">
        ...
        <p>各種手当: {{ detail.base_salary.toLocaleString() }} 円</p>
        <p>交通費: {{ detail.base_salary.toLocaleString() }} 円</p>
        <p>立替経費: {{ detail.base_salary.toLocaleString() }} 円</p>
        <p>その他収入: {{ detail.base_salary.toLocaleString() }} 円</p>
        <!-- いか必要な項目を入れる -->
    </div>
