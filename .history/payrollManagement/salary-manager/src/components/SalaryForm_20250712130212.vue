...
console.log('[② query]', { initialYear, initialMonth })
...
onMounted(async () => {
  if (id) {
    ...
    console.log('[③ API から取れた data]', data)
    // 数値が文字列で来た場合 Number() でキャスト
    // data が配列なら data[0] に
    const row = Array.isArray(data) ? data[0] : data
    if (!row) {
      console.warn('id はあるがレコードが無い')
      return
    }
    Object.assign(form, {
      ...data,
      base_salary:        Number(data.base_salary)        || 0,
      overtime_pay:       Number(data.overtime_pay)       || 0,
      allowances:         Number(data.allowances)         || 0,
      transport:          Number(data.transport)          || 0,
      expense_reimburse:  Number(data.expense_reimburse)  || 0,
      income_other:       Number(data.income_other)       || 0,
      health_insurance:   Number(data.health_insurance)   || 0,
      pension:            Number(data.pension)            || 0,
      employment_insurance: Number(data.employment_insurance) || 0,
      nursing_insurance:  Number(data.nursing_insurance)  || 0,
      social_insurance:   Number(data.social_insurance)   || 0,
      income_tax:         Number(data.income_tax)         || 0,
      resident_tax:       Number(data.resident_tax)       || 0,
      deduction_other:    Number(data.deduction_other)    || 0,
      refund:             Number(data.refund)             || 0,
      working_days:       Number(data.working_days)       || 0,
      paid_leave:         Number(data.paid_leave)         || 0,
      working_hours:      Number(data.working_hours)      || 0
    })
    Object.assign(form, row)
    console.log('[④ Object.assign 後の form]', { ...form })
  }
})
