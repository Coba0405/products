<template>
    <div class="p-6">
        ...
        <table class="w-full table-auto border-collapse">
            ...
            <tbody>
                <tr v-for="row in monthlyData" :key="row.month" class="hover:bg-gray-100 cursor-pointer" @click="$emit('select-month', row.month)">
...
<script setup>
    ...
    onMounted(async () => {
        try {
            const res =await axios.get('http://127.0.0.1:5000/api/salaries', {params: {}})
            const raw = res.data
