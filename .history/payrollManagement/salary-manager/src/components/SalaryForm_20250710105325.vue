<template>
    <form @submit.prevent="submitForm">
        ...
        <div class="mb-2">
            <label>
              <selsect v-model ="form.month" required @change="isMonthSelected = true">
                <option v-for="m in months" :key="month" :value="m">{{ m }}</option>
              </selsect>
            </label>
            <label>
              <select v-model ="form.base_salary" required>
                <option v-for="bs in base_salary" :key="base_salary" :value="bs">{{ bs }}</option>
              </select>
            </label>
        </div>
