  ...
  <div class="border p-6 rounded shadow-md bg-white max-w-2xl mx-auto">
    ...
    <form @submit.prevent="submitForm" class="text-flex">
      ...
      <div class="flex items-center gap-4">
        <label class="block text-sm mb-1 mr-10">会社名</label>
        <input v-model="form.company" type="text" class="input flex-1 rounded px-2 py-1 bg-gray-300" required />
      </div>

      <h2 class="mt-5">支給</h2>

      <div class="flex items-center gap-4">
        <label class="block text-sm mb-1 mr-10">基本給</label>
        <input v-model.number="form.base_salary" type="number" class="input flex-1 rounded px-2 py-1 bg-gray-300" />
      </div>

      <div class="flex items-center gap-4">
        <label class="block text-sm mb-1 mr-10">時間外勤務</label>
        <input v-model.number="form.overtime_pay" type="number" class="input flex-1 rounded px bg-gray-300" />
      </div>
