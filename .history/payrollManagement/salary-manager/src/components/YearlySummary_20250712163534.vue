...
<script setup>
    ...
    async function loadYearData () {
        try {
            ...
            const { data: raw } = await axios.get('/api/salaries', { params:{ year: currentYear.value } })
    ...
    function changeYear(delta){ currentYear.value += }
    // // データ取得
    // watchEffect(async () => {
    //     const year = currentYear.value          // 依存 ― 年が変われば再実行
    //     try {
